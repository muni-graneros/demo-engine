...
export async function portada(page, { titulo, subtitulo = '', capitulo = '', marca = {}, esperaMs = 2600 }) {
    ...
    await page.evaluate(({ titulo, subtitulo, capitulo, marca }) => {
        ...
        <div style="height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;
          ...
          ${capitulo ? `<div style="font-size:20px;letter-spacing:.28em;opacity:.75">CAPÍTULO ${capitulo}</div>` : ''}
          <h1 style="font-size:64px;margin:0;font-weight:700">${titulo}</h1>
          ${subtitulo ? `<p style="font-size:30px;margin:0;opacity:.9">${subtitulo}</p>` : ''}
          <div style="margin-top:26px;font-size:17px;opacity:.6">${marca.nombre ?? ''}</div>
        </div>`;
